...
class LocalUpdateSCDepth(object):
    def __init__(
        self,
        hparams,
        idx,
        ...
        self.id = idx  # user id
        self.dm = VideosDataModule(hparams)
        self.device = "cpu"
    ...
    def update_ssl_weights(
        ...
        """Train the local model with self-supervised learning"""
        ...
        trainer = Trainer(
            ...
            limit_train_batches=self.hparams.epoch_size,
            limit_val_batches=self.hparams.val_epoch_size,
            num_sanity_val_steps=5,
            benchmark=True
        )
        trainer.fit(model, self.dm)
        
        return model.state_dict(), sum(model.epoch_losses) / len(model.epoch_losses)
    ...
    def inference(self, model, test_dataset=None, test_user=None):
        ...
        results = trainer.test(model, self.data_module)
